...
class ScheduleForm(FlaskForm):
    ...
    address = StringField("Адрес", validators=[Optional(), Length(max=160)])
    type = SelectField("Тип", choices=[("Лекция", "Лекция"), ("Практическое занятие", "Практическое занятие"), ("Лабораторное занятие", "Лабораторное занятие"), ("Экзамен", "Экзамен")])
    group_name = StringField("Группа", validators=[Optional(), Length(max=80)])
    description = TextAreaField("Описание", validators=[Optional()])
    submit = SubmitField("Сохранить")
