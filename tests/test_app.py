...
from app.models import Event, Material, MaterialComment, Notification, ScheduleItem, Subject, Tag, User
...
def test_calendar_is_continuous_and_schedule_highlights_every_row(client, app):
    with app.app_context():
        db.session.add(ScheduleItem(date=date.today(), start_time=date.min.replace().time(), subject="Алгебра", group_name="ИВ-23"))
        db.session.commit()
    login(client, "Ivanov", "ИВ-23")
    response = client.get("/")
    assert response.status_code == 200
